...
const FileUploadModal: React.FC<FileUploadModalProps> = ({
  ...
  const { uploading, progressData, progressModalVisible, uploadDocuments, uploadImages, uploadWebPage } =
    useDocumentUpload();
  ...
  const beforeUpload = (file: File) => {
    const supportedTypes = SUPPORTED_FILE_TYPES[formatType] || [];
    const isValidType = supportedTypes.some((ext: string) =>
      file.name.toLowerCase().endsWith(ext)
    );
    if (!isValidType) {
      message.error(`不支持的文件类型，仅支持: ${supportedTypes.join(', ')}`);
